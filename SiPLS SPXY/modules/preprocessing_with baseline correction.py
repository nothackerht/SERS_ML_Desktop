...
class Preprocessing:
    ...
    def baseline_correction(self, spectra=None):
        """
        Perform baseline correction using Savitzky-Golay smoothing.

        Parameters:
        spectra (np.ndarray): Optional. If None, uses self.spectra. Shape = (wavelengths, spectra)
        ...
        for i in range(spectra.shape[1]):  # for each column/spectrum
            baseline = savgol_filter(spectra[:, i], window_length=51, polyorder=3)
