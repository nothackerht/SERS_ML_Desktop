...
# -*- coding: utf-8 -*-
"""
Created on Wed Jul  9 14:36:20 2025
...
class RegressionModel:
    ...
    @staticmethod
    def baseline_correction_polynomial(spectrum, poly_order=3, window_size=101):
        baseline = savgol_filter(spectrum, window_size, poly_order)
    ...
    def preprocess_and_filter_data(self, target_index):
        self.target_index = target_index  # store for downstream access
        ...
        targets = ['target_SI', 'HGS_pp_avg', 'ADF_pp_avg']
        y = df[targets[self.target_index]].values.reshape(-1, 1)
    ...
    def sipls_single_run_with_log(
        ...
        for (s, e) in all_intervals:
            ...
            for train_idx, val_idx in gkf.split(X_train[:, cols], y_train, groups=groups):
                ...
                from modules.interval_shell import get_model_by_name
                model = get_model_by_name(model_name, hyperparams)
        ...
        for k in range(1, max_combo_intervals+1):
            for combo in combinations(filtered_intervals, k):
                ...
                for fold, (tr_idx, va_idx) in enumerate(gkf.split(X_train[:,cols], y_train, groups=groups), 1):
                    ...
                    from modules.interval_shell import get_model_by_name
                    model = get_model_by_name(model_name, hyperparams or {'n_components': max_components, 'device': 'cuda'})
                    model.fit(Xtr, ytr)
                    yva_pred = model.predict(Xva)

                    # Group spectrum-level predictions and labels into per-sample (9 spectra per sample)
                    n_val_samples = yva.shape[0] // 9
                    yva_grouped   = yva.reshape(n_val_samples, 9, -1).mean(axis=1)
                    pred_grouped  = yva_pred.reshape(n_val_samples, 9, -1).mean(axis=1)
                    
                    # Compute R² based on sample-averaged predictions
                    fold_r2 = r2_score(yva_grouped, pred_grouped, multioutput='raw_values').mean()
        ...
        model_full = get_model_by_name(model_name, hyperparams or {'n_components': max_components, 'device': 'cuda'})
        model_full.fit(X_train[:, best_cols], y_train)
        y_pred = model_full.predict(X_test[:, best_cols])
    ...
    def evaluate_test_set_with_sipls_spxy(
        ...
        for methods in preprocess_methods_list:
            ...
            # ── 1) SPXY split ─────────────────────────────────────────────────────────
            X_raw, y_full = self.preprocess_and_filter_data(target_index)
            X_all   = X_raw.T
...
# BASELINE CORRECTION
# Apply baseline correction BEFORE any preprocessing
            # X_tr_raw = np.array([self.baseline_correction_polynomial(s) for s in X_tr_raw])
            # X_va_raw = np.array([self.baseline_correction_polynomial(s) for s in X_va_raw])
            ...
            # Light hyperparameter tuning grid per model
            TUNING_GRID = {
                'sipls': [
                    {'n_components': c, 'device': 'cuda'} for c in n_components_list
                ],
                'random_forest': [
            ...
            def _eval_combo(C, I, hp_override=None):
                try:
                    current_hyperparams = dict(hyperparams or {})
                    if hp_override is not None:
                        current_hyperparams.update(hp_override)
                    if model_name == 'sipls':
                        current_hyperparams['n_components'] = C
                        current_hyperparams.setdefault('device', 'cuda')
            
                    y_val_pred, combo, *_ = self.sipls_single_run_with_log(
                        X_tr, y_tr, X_va, y_va,
                        max_components=C if C is not None else 0,
                        num_intervals=I,
                        max_combo_intervals=max_combo_intervals,
            ...
            if model_name == 'sipls':
                ...
                # For nonlinear models, grid search over I and hyperparameters
                all_results = Parallel(n_jobs=-1, verbose=10)(
                    delayed(_eval_combo)(None, I, deepcopy(hp))
                    for I in num_intervals_list
                    for hp in TUNING_GRID.get(model_name, [hyperparams or {}])
                )
            ...
            best = min(grid, key=lambda x: x['val_RMSE'])
            I_opt, best_combo = best['I'], best['combo']
            C_opt = best.get('C', None)  # SiPLS only
            ...
            model_cal = get_model_by_name(model_name, hyperparams or {'n_components': C_opt, 'device': 'cuda'})
            model_cal.fit(X_tr[:, cols], y_tr)
            y_tr_pred = model_cal.predict(X_tr[:, cols])
            ...
            current_best_rmse = best['val_RMSE']
             # start from your validation RMSE on the original best_combo
            ...
            for i, (s, e) in enumerate(final_combo):
                ...
                for delta_s in range(-max_shift, max_shift+1):
                    for delta_e in range(-max_shift, max_shift+1):
                        ...
                        # fit & evaluate
                        model = get_model_by_name(model_name, hyperparams or {'n_components': C_opt, 'device': 'cuda'})
                        model.fit(X_tr[:, cols], y_tr)
                        yv = model.predict(X_va[:, cols])
            ...
            cols_full = np.hstack([np.arange(a, b) for (a, b) in final_combo])
            model_full = get_model_by_name(model_name, hyperparams or {'n_components': C_opt, 'device': 'cuda'})
            model_full.fit(X_all[:, cols_full], y_full)
            y_all_pred = model_full.predict(X_all[:, cols_full])
            ...
            self.plot_training_spectra_with_intervals(
                preprocessing_name=f"{name} | {model_name}",
                wavenumbers=self.wavenumbers,
                X_train=X_tr,
            ...
            # # Apply baseline correction to each test spectrum
            # X_test = np.array([
            #     self.baseline_correction_polynomial(s) for s in test_raw.T
            # ])
            ...
            y_spec   = model_full.predict(X_test[:, cols_full])
            ...
            avg_pred_si = reshaped_preds.mean(axis=1).flatten()
            std_pred_si = reshaped_preds.std(axis=1).flatten()

            
            # Get true SI values and mask for valid (non-NaN) ones
            target_cols = ['target_SI', 'HGS_pp_avg', 'ADF_pp_avg']
            target_col = target_cols[target_index]
            true_si = pd.to_numeric(test_meta[target_col].values[:n_test], errors='coerce')


            # 🔍 INSERT THESE DEBUG PRINTS HERE
            print("target_index:", target_index)
            print("test_meta columns:", test_meta.columns)
            print("First few rows:\n", test_meta.head())
            print("true_si before masking:", true_si)
            print("Number of valid (non-NaN) entries:", np.sum(~np.isnan(true_si)))
            ...
            pred_df = pd.DataFrame({
                ...
                f'True_{target_name}': true_si,
                f'Predicted_{target_name}': avg_pred_si,
                'Std_Dev': std_pred_si
            })
            ...
            # ── parity plot & summary ───────────────────────────────────────────────
            # Log best hyperparameters for this preprocessing
            summary = pd.DataFrame([
                {'Set': 'Calibration',     'RMSE': train_rmse,         'R2': train_r2.mean()},
                {'Set': 'SPXY-Validation', 'RMSE': best['val_RMSE'],   'R2': best['val_R2']},
                {'Set': 'Retrain',         'RMSE': retrain_rmse,       'R2': retrain_r2.mean()},
                {'Set': 'Blind test',      'RMSE': rmse_test,          'R2': r2_test},
            ])
            
            # Add metadata columns for hyperparameters, model, and preprocessing
            for col in summary.columns:
                if col not in ['Set', 'RMSE', 'R2']:
                    continue  # preserve existing structure
            
            summary['Preprocessing'] = name
            summary['Model'] = model_name
            summary['Best_Combo_Intervals'] = str(final_combo)
            
            # Also add hyperparameter info
            if model_name != 'sipls':
                best_hyperparams = {**(hyperparams or {}), **(best.get('hp_override', {}) if isinstance(best.get('hp_override', {}), dict) else {})}
                for k, v in best_hyperparams.items():
                    summary[f'HP_{k}'] = v
            else:
                summary['HP_n_components'] = C_opt


            summary.to_excel(os.path.join(out_dir, f'AllMetrics_{target_name}.xlsx'), index=False)
            ...
            plt.title(f"{target_name} | {name} | Model={model_name} | C={C_opt} I={I_opt} | R²={r2_test:.3f}", fontsize=14)

            plt.tight_layout()
            plt.savefig(os.path.join(out_dir, f'Parity_{target_name}_{model_name}.png'), dpi=300)
        ...
        best_r2 = best['val_R2']  # ✅ FIX: define best_r2

        print(f" Per-sample predictions saved to:\n{preds_path}")
        print(f"\n✅ {target_name} | {name}")
        print(f"  - Calibration R²:     {train_r2.mean():.3f}")
        print(f"  - Validation R²:      {best_r2:.3f}")
        print(f"  - Retrained R²:       {retrain_r2.mean():.3f}")
        print(f"  - Blind Test R²:      {r2_test:.3f}")
        print(f"  - Blind Test RMSE:    {rmse_test:.4f}")
        print(f"  - Parity plot saved to: {os.path.join(out_dir, f'Parity_{target_name}.png')}")
    ...
    def plot_training_spectra_with_intervals(
        ...
        if output_path:
            ...
            new_path = f"{base}_model={model_name}{ext}"

            plt.savefig(new_path, dpi=300, bbox_inches='tight')


        plt.show()
def run_interval_selection_and_modeling(
    all_spectra,
    y_label_df,
    wavenumbers,
    target_name='SI',
    test_spectra_path=None,
    test_metadata_path=None,
    output_dir=".",
    model_list=None
):
    # ✅ Global settings (defined inside function body)
    GLOBAL_NUM_INTERVALS_LIST = [10]
    GLOBAL_MAX_SHIFT = 20
    GLOBAL_MAX_COMBO_INTERVALS = 4

    target_map = {'SI': 0, 'HGS': 1, 'ADF': 2}
    target_index = target_map[target_name]

    if model_list is None:
        model_list = ['sipls', 'random_forest', 'xgboost', 'svr', 'mlp', 'knn', 'gpr']

    regressor = RegressionModel(all_spectra, y_label_df, wavenumbers)

    for model in model_list:
        print(f"\n🔍 Running interval selection + evaluation for model: {model.upper()} ...")
        regressor.evaluate_test_set_with_sipls_spxy(
            test_spectra_path=test_spectra_path,
            test_metadata_path=test_metadata_path,
            target_index=target_index,
            target_name=target_name,
            output_dir=os.path.join(output_dir, model),
            model_name=model,
            num_intervals_list=GLOBAL_NUM_INTERVALS_LIST,
            max_shift=GLOBAL_MAX_SHIFT,
            max_combo_intervals=GLOBAL_MAX_COMBO_INTERVALS
        )


"""Pipeline Overview:
------------------
The pipeline performs the following steps for each specified preprocessing configuration:

1. **Data Preparation**
   - Loads spectral and metadata files.
   - Filters to include only relevant classes (e.g., 'Control' and 'DM1').
   - Assigns synthetic target values to controls (e.g., SI = 0).
   - Flattens the sample-level data structure to individual spectra (9 spectra/sample).

2. **SPXY Sample-wise Split**
   - Performs SPXY (Sample-based) split using Euclidean distance on both X and y.
   - Splits the dataset into 80% training and 20% validation (by sample, preserving group structure).

3. **Preprocessing**
   - Applies selected preprocessing method(s), such as:
     - EMSC (Extended Multiplicative Signal Correction)
     - Normalization
     - SNV (Standard Normal Variate)
     - Second Derivative
   - Optionally allows baseline correction to be applied beforehand (commented for flexibility).

4. **SiPLS Hyperparameter Optimization via Grid Search**
   - Grid-searches over:
     - Number of latent components (n_components_list)
     - Number of wavenumber intervals (num_intervals_list)
   - For each setting:
     - Splits training data into 5 inner folds using GroupKFold (sample-level grouping).
     - Evaluates performance (R²) of each individual interval and ranks top-k intervals.
     - Exhaustively combines top-k intervals up to a defined max_combo_intervals.
     - Selects the best interval combination based on sample-averaged R².

5. **Interval Expansion**
   - Expands each interval from the best combination by ±max_shift wavenumber indices.
   - Keeps expansions that improve validation RMSE.

6. **Model Retraining**
   - Retrains the model using the full SPXY (train + val) data and the expanded intervals.
   - Computes final training performance metrics (RMSE, R²).

7. **Blinded Test Set Evaluation**
   - Loads the external blinded test set (spectra and metadata).
   - Applies the same preprocessing steps using training-based references (e.g., EMSC mean).
   - Predicts all 9 spectra per test sample and averages them to obtain per-sample predictions.
   - Computes RMSE and R² for the test set and logs error bars.

8. **Results Saving**
   - Saves parity plots, interval diagrams, test results, and logs in a dedicated output folder.
   - Merges all results into:
     - Combined_Model_Performance.xlsx
     - All_Predictions_PerSample.xlsx"""