# -*- coding: utf-8 -*-
"""
SPXY evaluation that REPLAYS EVERY COMBINATION listed in leaderboard CSVs
(model × preprocessing × HP) and picks a SPXY winner.

Flip the PRESET (one line) to switch:
- Boxes 1–3 vs 1–2
- With vs Without Controls

This file adds the project root to sys.path so 'modules.*' imports resolve.
"""
...
# - Boxes 1–3 (with & without controls) read CSVs from COmbinedCVresultsnocontrol\<target>\
# - Boxes 1–2 (with & without controls) read CSVs from box1and2withcontrols\<target>\

PRESETS = {
    "B123_with_controls": {
        "TRAIN_DATA_DIR": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\data_combined",
        "TRAIN_META_PATH": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\y_metadata_combined_in_order.csv",
        "INCLUDE_TYPES": ("DM1", "Control"),
        # READ combos (CSV) - per-target directories (same as "without controls" by your instruction)
        "GLOBAL_RESULTS_DIRS": {
        ...
        # WRITE outputs
        "OUT_DIR_SPXY": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\spxy_eval\Boxes1-3 with controls no leak",
    },
    "B123_no_controls": {
        "TRAIN_DATA_DIR": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\data_combined",
        "TRAIN_META_PATH": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\y_metadata_combined_in_order.csv",
        "INCLUDE_TYPES": ("DM1",),
        # READ combos (CSV) - per-target directories (same as "with controls" by your instruction)
        "GLOBAL_RESULTS_DIRS": {
            "ADF_pp_avg":  r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\ten_fold_cv_global_combined\COmbinedCVresultsnocontrol\ADF_pp_avg",
            "HGS_pp_avg":  r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\ten_fold_cv_global_combined\COmbinedCVresultsnocontrol\HGS_pp_avg",
            "target_SI":   r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\ten_fold_cv_global_combined\COmbinedCVresultsnocontrol\target_SI",
        ...
        # WRITE outputs
        "OUT_DIR_SPXY": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\spxy_eval\Boxes1-3 without controls no leak",
    },
    "B12_with_controls": {
        ...
        "INCLUDE_TYPES": ("DM1", "Control"),
        # READ combos (CSV) - per-target directories (same for with & without controls)
        "GLOBAL_RESULTS_DIRS": {
        ...
        # WRITE outputs
        "OUT_DIR_SPXY": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\spxy_eval\WITH CONTROLS NO LEAK",
    },
    "B12_no_controls": {
        "TRAIN_DATA_DIR": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\data",
        "TRAIN_META_PATH": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\Data\y_metadata.csv",
        ...
        # WRITE outputs
        "OUT_DIR_SPXY": r"C:\Users\spect\Desktop\MD-Analysis-main (3)\SERS_ML_Desktop\spxy_eval\wITHOUT CONTROLSNO LEAK",
    },
}

# <<< flip this one line to switch scenarios
# PRESET = "B12_with_controls"
# PRESET = "B12_no_controls"
PRESET = "B123_with_controls"
# PRESET = "B123_no_controls"


cfg = PRESETS[PRESET]
TRAIN_DATA_DIR   = cfg["TRAIN_DATA_DIR"]
TRAIN_META_PATH  = cfg["TRAIN_META_PATH"]
INCLUDE_TYPES    = cfg["INCLUDE_TYPES"]
RESULT_DIRS_MAP  = {k: Path(v) for k, v in cfg["GLOBAL_RESULTS_DIRS"].items()}
OUT_DIR_SPXY     = cfg["OUT_DIR_SPXY"]
...
def _load_combos_from_csv_dir(results_dir: Path) -> pd.DataFrame:
    ...
    * *_metrics*_all_combos*.csv/.xlsx/.xls
...
def main():
    ...
    for nice, tcol in TARGETS:
        print(f"\n=== SPXY (replay ALL combos) for {nice} ({tcol}) ===")
        ...
        for r in range(N_REPEATS):
            ...
            spxy_rows = []
            best = None  # (rmse, model, methods_list, hp_str, yval_pred, yval_pred_sd)
            ...
            for _, row in combos_df.iterrows():
                ...
                # track best by SPXY RMSE
                if (best is None) or (mets["rmse"] < best[0]):
                    best = (mets["rmse"], model_name, methods, hp_str, yval_pred, yval_pred_sd)
            ...
            spxy_metrics = (
                ...
                .reset_index(drop=True)
                if spxy_rows else
                pd.DataFrame(columns=["repeat","target","model","preprocessing","hp","rmse","r2","mae","medae","evs"])
            )
            spxy_metrics_path = os.path.join(tgt_dir, f"{tcol}__metrics_all_combos_SPXY_r{r+1}.xlsx")
            spxy_metrics.to_excel(spxy_metrics_path, index=False)
            print(f"[SPXY-METRICS] wrote {spxy_metrics_path}  ({len(spxy_metrics)} combos)")

            # winner artifacts
            if best is not None:
                _, model_best, methods_best, hp_best, y_pred_best, y_pred_sd_best = best
                ...
                title_suffix  = f"{model_best.upper()} | {methods_label} | HP: {hp_best} | SPXY {int(CAL_FRAC*100)}/{int((1-CAL_FRAC)*100)}"
                ...
                save_outer_predictions_excel(
                    ...
                    meta_kept=meta_val,
                    target_order=(tcol,),
                    out_path=os.path.join(winner_dir, f"{tcol}__winner_SPXY_predictions_r{r+1}.xlsx"),
                    y_true_sample_std=None,
                    y_pred_sample_std=np.asarray(y_pred_sd_best).reshape(-1,1),
                )
            ...
            if best is not None:
                best_rmse = float(best[0])
                summary_rows.append({
                    "target": tcol,
                    "repeat": r+1,
                    "best_model": best[1],
                    "best_preprocessing": ("+".join(best[2]) if best[2] else "None"),
                    "best_hp": best[3],
                    "rmse": best_rmse
                })
    ...
    if summary_rows:
        ...
        out_sum = os.path.join(OUT_DIR_SPXY, "spxy_summary.xlsx")
